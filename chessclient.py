...
class ChessClient:
    ...
    def recv_length_prefixed(self):
        length = int.from_bytes(self.socket.recv(1), ENDIANNESS)
        return self.socket.recv(length)

    def run(self):
        self.send_length_prefixed(str(self.opponent).encode('utf-8'))
        if self.opponent == "Network":
            print("Waiting for another network opponent to  connect...")
        while True:
            data = self.recv_length_prefixed().decode("utf-8")
            if len(data) == 1:
                break
            print(data)

            move = self.player.get_move(data)
            self.send_length_prefixed(str(move).encode('utf-8'))
        ...
        url = self.recv_length_prefixed().decode("utf-8")
